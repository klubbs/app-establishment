...
export async function mergeStoreInStorage(store: TStoreResponse) {
	await AsyncStorage.mergeItem("@KLUBBS_STORE:KEY", JSON.stringify(store));
}
...
export const getStoreInStorage = async (): Promise<TStoreResponse | null> => {
	...
	return JSON.parse(result as any) as TStoreResponse;
};
